...
void CWing::Duplicate(CWing *pWing)
{
	...
	//
        ...
        ////////////new code DM//////////
        for (int i = 0; i< pWing->m_Polar.size();i++)
                     {
            m_Polar.append(pWing->m_Polar.at(i));
        }
        blades = pWing->blades;
        /////////end new code DM//////
...
//	int iSurface = 0;
        ...
        for(j=startIndex; j<m_NSurfaces; j++)
	{
		...
		out << ("\n");
		out << ("AFIL 0.0 1.0\n");
        out << (ASurface.m_pFoilA->getName() +".dat\n");
		out << ("\n");
...
*/
	...
	if(!ASurface.m_bTEFlap)
	{
		...
		out << ("\n");
		out << ("AFIL 0.0 1.0\n");
        out << (ASurface.m_pFoilB->getName() +".dat\n");
		out << ("\n");
	}
...
void CWing::PanelComputeViscous(double QInf, double /*Alpha*/, CWPolar *pWPolar, double &WingVDrag, bool bViscous, QString &OutString)
{
	...
	for (int j=0; j<m_NSurfaces; j++)
	{
		for(int k=0; k<m_Surface[j].m_NYPanels; k++)
		{
			...
			m_PCd[m]    = GetVar(&g_polarStore, 2, m_Surface[j].m_pFoilA, m_Surface[j].m_pFoilB, m_Re[m], m_Cl[m], tau, bOutRe, bError);
			bPointOutRe = bOutRe || bPointOutRe;
			if(bError) bPointOutCl = true;
			m_XTrTop[m] = GetVar(&g_polarStore, 5, m_Surface[j].m_pFoilA, m_Surface[j].m_pFoilB, m_Re[m], m_Cl[m], tau, bOutRe, bError);
			bPointOutRe = bOutRe || bPointOutRe;
			if(bError) bPointOutCl = true;
			m_XTrBot[m] = GetVar(&g_polarStore, 6, m_Surface[j].m_pFoilA, m_Surface[j].m_pFoilB, m_Re[m], m_Cl[m], tau, bOutRe, bError);
			bPointOutRe = bOutRe || bPointOutRe;
			if(bError) bPointOutCl = true;
			if(bPointOutCl)
			{
