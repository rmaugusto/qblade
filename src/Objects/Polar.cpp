...
//        ar << m_ACrit << m_XTop << m_XBot;  // NM END
	}
    else
    {
		//read variables
...
//		setParentName(strong);  // NM REPLACE
		setSingleParent(g_mainFrame->GetFoil(strong));

		ReadCString(ar, strong);
		setName(strong);

		if(getParent() == NULL || getName() == "" ) {
			return;
		}
