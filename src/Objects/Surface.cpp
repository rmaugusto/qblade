...
void CSurface::SetTwist3(double /*x*/, double /*z*/)
{
    double xc4,zc4;
    CVector O(0.0,0.0,0.0);
    ...
    //"A" section first
    xc4 = m_LA.x + (m_TA.x-m_LA.x)*m_XPitchA;
    zc4 = m_LA.z + (m_TA.x-m_LA.x)*m_ZPitchA;
	m_LA.x = xc4 + (LA.x-xc4) * cos(m_TwistA *PI/180.0) + (LA.z-zc4) * sin(m_TwistA *PI/180.0);
    m_LA.z = zc4 - (LA.x-xc4) * sin(m_TwistA *PI/180.0) + (LA.z-zc4) * cos(m_TwistA *PI/180.0);
	m_TA.x = xc4 + (TA.x-xc4) * cos(m_TwistA *PI/180.0) + (TA.z-zc4) * sin(m_TwistA *PI/180.0);
    m_TA.z = zc4 - (TA.x-xc4) * sin(m_TwistA *PI/180.0) + (TA.z-zc4) * cos(m_TwistA *PI/180.0);
//    NormalA.RotateY(O, m_TwistA);
    ...
    //"B" Section next
    xc4 = m_LB.x + (m_TB.x-m_LB.x)*m_XPitchB;
    zc4 = m_LB.z + (m_TB.x-m_LB.x)*m_ZPitchB;
	m_LB.x = xc4 + (LB.x-xc4) * cos(m_TwistB *PI/180.0) + (LB.z-zc4) * sin(m_TwistB *PI/180.0);
    m_LB.z = zc4 - (LB.x-xc4) * sin(m_TwistB *PI/180.0) + (LB.z-zc4) * cos(m_TwistB *PI/180.0);;
	m_TB.x = xc4 + (TB.x-xc4) * cos(m_TwistB *PI/180.0) + (TB.z-zc4) * sin(m_TwistB *PI/180.0);;
    m_TB.z = zc4 - (TB.x-xc4) * sin(m_TwistB *PI/180.0) + (TB.z-zc4) * cos(m_TwistB *PI/180.0);;
...
//    L2=CVector(0,0,1);
//    L1=CVector(0,m_VecA.y,m_VecA.z);

//    angle = -acos(L1.dot(L2)/L1.VAbs()/L2.VAbs());
//    angle+=90.0/180.0*PI;

//    NormalA.RotateX(O,angle*180/PI);

//    //"B" Section next

//    L2=CVector(0,0,1);
//    L1=CVector(0,m_VecB.y,m_VecB.z);

//    angle = -acos(L1.dot(L2)/L1.VAbs()/L2.VAbs());
//    angle+=90.0/180.0*PI;

//    NormalB.RotateX(O,angle*180/PI);

    // first we rotate around the z-axis

    //"A" section first

    L2=CVector(m_LA.x-m_TA.x,0,0);
    L1=CVector(m_VecA.x,m_VecA.y,0);
    ...
    xc4 = m_LA.x + (m_TA.x-m_LA.x)*m_XPitchA;
    zc4 = m_LA.y;
    m_LA.x = xc4 + (LA.x-xc4) * cos(angle) - (LA.y-zc4) * sin(angle);
    m_LA.y = zc4 - (LA.x-xc4) * sin(angle) + (LA.y-zc4) * cos(angle);
    m_TA.x = xc4 + (TA.x-xc4) * cos(angle) - (TA.y-zc4) * sin(angle);
    m_TA.y = zc4 - (TA.x-xc4) * sin(angle) + (TA.y-zc4) * cos(angle);
//    NormalA.RotateZ(O, angle *180/PI);
    ...
    L2=CVector(m_LB.x-m_TB.x,0,0);
    L1=CVector(m_VecB.x,m_VecB.y,0);
    ...
    xc4 = m_LB.x + (m_TB.x-m_LB.x)*m_XPitchB;
    zc4 = m_LB.y;
    m_LB.x = xc4 + (LB.x-xc4) * cos(angle) - (LB.y-zc4) * sin(angle);
    m_LB.y = zc4 - (LB.x-xc4) * sin(angle) + (LB.y-zc4) * cos(angle);;
    m_TB.x = xc4 + (TB.x-xc4) * cos(angle) - (TB.y-zc4) * sin(angle);;
    m_TB.y = zc4 - (TB.x-xc4) * sin(angle) + (TB.y-zc4) * cos(angle);;
//    NormalB.RotateZ(O, angle *180/PI);
