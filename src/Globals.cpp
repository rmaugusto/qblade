...
void SetUnits(int LUnit, int AUnit, int SUnit, int WUnit, int FUnit, int MUnit, int PUnit,
                          double &mtoUnit, double &m2toUnit, double &mstoUnit,  double &kgtoUnit, double &NtoUnit, double &NmtoUnit, double &WtoUnit)
{
	switch(LUnit)
	{
		case 0:
...
*/
	...
	for (i = 0; i< m_poaPolar->size(); i++)
	{
		...
		if((pPolar->m_PolarType==FIXEDSPEEDPOLAR) && (pPolar->getParent() == pFoil))
		{
			n++;
			if(n>=2) break;
		}
...
//more than one polar - interpolate between  - tough job
	...
	{
		pPolar = m_poaPolar->at(i);
		if((pPolar->m_PolarType==FIXEDSPEEDPOLAR) && (pPolar->getParent() == pFoil) && pPolar->m_Alpha.size()>0)
		{
...
//more than one polar - interpolate between  - tough job
	...
	{
		pPolar = (CPolar*)m_poaPolar->at(i);
		if((pPolar->m_PolarType==FIXEDSPEEDPOLAR) && (pPolar->getParent() == pFoil) && pPolar->m_Cl.size()>0)
		{
			// we have found the first type 1 polar for this foil
			if (Re < pPolar->m_Reynolds)
			{
...
double GetZeroLiftAngle(Store<CPolar>*m_poaPolar, CFoil *pFoil0, CFoil *pFoil1, double Re, double Tau)
{
	...
	else
	{
		...
		{
			pPolar = (CPolar*)m_poaPolar->at(i);
			if(pPolar->getParent() == pFoil0) size++;
		}
		if(size)
		{
			for (i=0; i<m_poaPolar->size(); i++)
