...
bool QXInverse::InitXFoil(CFoil * pFoil)
{
    ...
    m_pModFoil->setName(pFoil->getName() + tr(" Modified"));
    pXFoil->m_FoilName = m_pRefFoil->getName() ;
...
void QXInverse::OnExtractFoil()
{
	...
	if(m_bLoaded)
	{
        dlg.m_FoilName = m_pRefFoil->getName();
	}
	...
	if(QDialog::Accepted == dlg.exec())
	{
        ...
        m_pModFoil->setName(m_pRefFoil->getName() + tr(" Modified"));
...
void QXInverse::OnStoreFoil()
{
    ...
    pFoil->setName(m_pRefFoil->getName());
    pMainFrame->StoreFoil(pFoil);
}
...
//draw the reference and modified foils  
	...
	if(m_bRefFoil && m_bLoaded)
	{
		...
		painter.setPen(TextPen);
        painter.drawText(50, m_rGraphRect.bottom()+25, m_pRefFoil->getName());
	}
	...
	if(m_bModFoil && m_bLoaded) 
	{
		...
		painter.setPen(TextPen);
        painter.drawText(50, m_rGraphRect.bottom()+40, m_pModFoil->getName());
	}
...
bool QXInverse::SetParams()
{
    ...
    if (g_pCurFoil && pXFoil->m_FoilName==g_pCurFoil->getName() && pXFoil->lqspec)
	{
...
//		InitXFoil(m_pRefFoil);
    ...
    m_pRefFoil->setName(pXFoil->m_FoilName);
	m_pRefFoil->InitFoil();
    m_pModFoil->setName(pXFoil->m_FoilName + tr(" Modified"));
...
//	move(r.width()/3, r.height()/6);
	...
	m_pctrlSpec   = new NumberEdit();
	m_pctrlSpec->setValue(1.23);
	...
	m_pctrlFilterParam = new NumberEdit();
	m_pctrlFilterParam->setValue(2.34);
	...
	m_pctrlTAngle = new NumberEdit(NumberEdit::Standard, 3);
	m_pctrlTAngle->setValue(3.681);
	m_pctrlTGapx  = new NumberEdit(NumberEdit::Standard, 3);
	m_pctrlTGapx->setValue(0.001);
	m_pctrlTGapy  = new NumberEdit(NumberEdit::Standard, 3);
	m_pctrlTGapy->setValue(0.002);
	...
	m_pctrlIter           = new NumberEdit();
	m_pctrlIter->setValue(11);
