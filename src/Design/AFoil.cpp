...
{
	int i;
	m_pFoilModel->setRowCount(g_foilStore.size()+1);
...
void QAFoil::FillTableRow(int row)
{
	QModelIndex ind;
	QStandardItem *pItem;
	CFoil *pFoil = g_foilStore.at(row-1);
...
void QAFoil::OnAFoilDerotateFoil()
{
    ...
    m_pBufferFoil->setName(g_pCurFoil->getName());
    ...
    CFoil * pFoil = pMainFrame->StoreFoil(pNewFoil);
	FillFoilTable();
	SelectFoil(pFoil);
...
void QAFoil::OnDuplicate()
{
    ...
    if(pMainFrame->StoreFoil(pNewFoil))
	{
		FillFoilTable();
		SelectFoil(pNewFoil);
	}
...
void QAFoil::OnImportFoil(){

    QString PathName;
    QXDirect *pXDirect = (QXDirect*) g_mainFrame->m_pXDirect;

    PathName = QFileDialog::getOpenFileName(this, tr("Open Foil File"),
    ...
    CFoil *pFoil = g_mainFrame->ReadFoilFile(ar);
    XFile.close();

    if(pFoil)
    {
        g_mainFrame->StoreFoil(pFoil);
        g_pCurFoil  = pFoil;
...
void QAFoil::OnHideAllFoils()
{
	...
	pMainFrame->SetSaveState(false);
	CFoil*pFoil;
	for (int k=0; k<g_foilStore.size(); k++)
	{
...
//					pDC->Rectangle(x1-2, Place.y + ypos*k-2, x1+2, Place.y + ypos*k+2);
		...
		k++;
		for (n=0; n < g_foilStore.size(); n++)
		{
			pRefFoil = g_foilStore.at(n);
			if(pRefFoil && pRefFoil->m_bVisible)
			{
                strong = pRefFoil->getName();
				if(strong.length())
				{
					...
					painter.setPen(TextPen);
                    painter.drawText(Place.x() + (int)(1.5*LegendSize), Place.y() + ypos*k+delta, pRefFoil->getName());
					k++;
				}
			}
		}
