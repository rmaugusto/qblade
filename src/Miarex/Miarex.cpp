...
//	m_WngAnalysis.m_NXWakePanels    = 1;
	...
	for(int ig=0; ig<4; ig++)
	{
		...
		m_WingGraph[ig].SetXMajGrid(true, QColor(200,200,200),2,1);
		m_WingGraph[ig].SetXMinGrid(false, true, QColor(80,80,80),2, 1, 100.0);
		m_WingGraph[ig].SetYMajGrid(true, QColor(200,200,200),2,1);
		m_WingGraph[ig].SetYMinGrid(false, true, QColor(80,80,80),2, 1, 0.1);
		m_WingGraph[ig].SetType(1);
		m_WingGraph[ig].SetMargin(50);

		m_WPlrGraph[ig].SetXMajGrid(true, QColor(200,200,200),2,1);
		m_WPlrGraph[ig].SetYMajGrid(true, QColor(200,200,200),2,1);
	...
	m_CpGraph.SetXMajGrid(true, QColor(200,200,200),2,1);
	m_CpGraph.SetYMajGrid(true, QColor(200,200,200),2,1);
	...
	for(int ig=0; ig<4; ig++)
	{
		m_TimeGraph[ig].SetXMajGrid(true, QColor(200,200,200),2,1);
		m_TimeGraph[ig].SetYMajGrid(true, QColor(200,200,200),2,1);
	...
	if(m_bLongitudinal)
	{
		...
		m_TimeGraph[2].SetYTitle("q ("+QString::fromUtf8("deg") +"/s)");
		m_TimeGraph[3].SetYTitle("theta ("+QString::fromUtf8("deg") +"/s)");
	}
	else
	{
		m_TimeGraph[0].SetYTitle("v (m/s)");
		m_TimeGraph[1].SetYTitle("p ("+QString::fromUtf8("deg") +"/s)");
		m_TimeGraph[2].SetYTitle("r ("+QString::fromUtf8("deg") +"/s)");
		m_TimeGraph[3].SetYTitle("phi ("+QString::fromUtf8("deg") +"/s)");
	}

	//set the default settings for the root locus graphs
	m_LongRLGraph.SetXMajGrid(true, QColor(200,200,200),2,1);
	m_LongRLGraph.SetYMajGrid(true, QColor(200,200,200),2,1);
	...
	m_LatRLGraph.SetXMajGrid(true, QColor(200,200,200),2,1);
	m_LatRLGraph.SetYMajGrid(true, QColor(200,200,200),2,1);
		...
		for (i=0; i<m_poaPOpp->size(); i++)
		{
			...
			if (pPOpp->m_PlaneName == pOldPOpp->m_PlaneName)
			{
				if (pPOpp->m_PlrName == pOldPOpp->m_PlrName)
				{
					...
					{
						if(fabs(pPOpp->m_QInf - pOldPOpp->m_QInf)<0.1)
						{
							//replace existing point
							pPOpp->m_Color = pOldPOpp->m_Color;
							pPOpp->m_Style = pOldPOpp->m_Style;
...
CPOpp * QMiarex::GetPOpp(double x)
{
	...
	for (i=0; i<m_poaPOpp->size(); i++)
	{
		...
		{
			if     (m_pCurWPolar->m_WPolarType< FIXEDAOAPOLAR && fabs(pPOpp->m_Alpha - x)<0.005)               return pPOpp;
			else if(m_pCurWPolar->m_WPolarType==FIXEDAOAPOLAR  && fabs(pPOpp->m_QInf - x)<0.005)  return pPOpp;
			else if(m_pCurWPolar->m_WPolarType==STABILITYPOLAR && fabs(pPOpp->m_Ctrl - x)<0.005)  return pPOpp;
		}
	}
	return NULL;
}
...
CWOpp* QMiarex::GetWOpp(double x)
{
	...
	for (i=0; i<m_poaWOpp->size(); i++)
	{
		...
		{
			if(m_pCurWPolar->m_WPolarType<FIXEDAOAPOLAR && fabs(pWOpp->m_Alpha - x)<0.005)                   return pWOpp;
			else if(m_pCurWPolar->m_WPolarType==FIXEDAOAPOLAR && fabs(pWOpp->m_QInf - x)<0.005)  return pWOpp;
			else if(m_pCurWPolar->m_WPolarType==STABILITYPOLAR && fabs(pWOpp->m_Ctrl - x)<0.005) return pWOpp;
		}
	}
	return NULL;
}
			...
			for(j=0; j<m_pWingList[iw]->m_NSurfaces; j++)
			{
				...
				if(pFoil)
				{
					pGLWidget->renderText(m_pWingList[iw]->m_Surface[j].m_TA.x, m_pWingList[iw]->m_Surface[j].m_TA.y, m_pWingList[iw]->m_Surface[j].m_TA.z,
                                            pFoil->getName(), pMainFrame->m_TextFont);
				}
			}
...
//		CPanel::s_CtrlPos   = pSettings->value("CtrlPos").toDouble();
//		CPanel::s_VortexPos = pSettings->value("VortexPos").toDouble();
...
void QMiarex::OnAdvancedSettings()
{
	...
	m_WAdvancedDlg.m_ControlPos      = CPanel::s_CtrlPos;
	m_WAdvancedDlg.m_VortexPos       = CPanel::s_VortexPos;
	...
	if(m_WAdvancedDlg.exec() == QDialog::Accepted)
	{
		...
		CPanel::s_CtrlPos      = m_WAdvancedDlg.m_ControlPos;
		CPanel::s_VortexPos    = m_WAdvancedDlg.m_VortexPos;
...
void QMiarex::OnDefineStabPolar()
{
	...
	m_StabPolarDlg.m_pBlade  = m_pWingList[0];
	...
	if(res == QDialog::Accepted)
	{
		...
		pCurWPolar->m_WPolarType      = m_StabPolarDlg.m_WPolarType;
...
void QMiarex::OnDefineWPolar()
{
	...
	m_WngAnalysis.m_pBlade       = m_pCurWing;
	...
	if (res == QDialog::Accepted)
	{
		...
		pNewWPolar->m_WPolarType      = m_WngAnalysis.m_WPolarType;
...
void QMiarex::OnManageUFOs()
{
	...
	else if(m_ManageUFOsDlg.m_pBlade) SetUFO(m_ManageUFOsDlg.m_pBlade->m_WingName);
	else                             SetUFO();
...
void QMiarex::OnUFOInertia()
{
	...
	else if(m_pCurWing)
	{
		...
		m_InertiaDlg.m_pBlade  = m_pCurWing;
	}
...
void QMiarex::PanelAnalyze(double V0, double VMax, double VDelta, bool bSequence)
{
	...
	m_pPanelDlg->m_pBlade          = m_pCurWing;
...
bool QMiarex::SaveSettings(QSettings *pSettings)
{
	...
	pSettings->beginGroup("Miarex");
	{
		...
		pSettings->setValue("CtrlPos",   CPanel::s_CtrlPos);
		pSettings->setValue("VortexPos", CPanel::s_VortexPos);
	...
	QGroupBox *AnalysisBox = new QGroupBox(tr("Analysis settings"));
	{
		QVBoxLayout *AnalysisGroupLayout = new QVBoxLayout;
		{
			...
			QGridLayout *SequenceGroupLayout = new QGridLayout;
			{
				...
				m_pctrlAlphaMin     = new NumberEdit(NumberEdit::Standard, 3);
				m_pctrlAlphaMax     = new NumberEdit(NumberEdit::Standard, 3);
				m_pctrlAlphaMax->setValue(1);
				m_pctrlAlphaDelta   = new NumberEdit(NumberEdit::Standard, 3);
				m_pctrlAlphaDelta->setValue(0.5);
	...
	QGroupBox *CpBox = new QGroupBox(tr("Cp Sections"));
	{
		QVBoxLayout *CpParams = new QVBoxLayout;
		{
			...
			QHBoxLayout *CpPos = new QHBoxLayout;
			{
				...
				m_pctrlSpanPos = new NumberEdit(NumberEdit::Standard, 3);
...
void QMiarex::SetStabGraphTitles()
{
	...
	if(m_bLongitudinal)
	{
		...
		m_TimeGraph[2].SetYTitle("q ("+QString::fromUtf8("deg") +"/s)");
		m_TimeGraph[3].SetYTitle("theta "+QString::fromUtf8("(deg)"));
	}
	else
	{
		...
		m_TimeGraph[1].SetYTitle("p ("+QString::fromUtf8("deg") +"/s)");
		m_TimeGraph[2].SetYTitle("r ("+QString::fromUtf8("deg") +"/s)");
		m_TimeGraph[3].SetYTitle("phi "+QString::fromUtf8("(deg)"));
	}
}
...
{
	//NM Miarex will be deleted anyway
//	MainFrame *pMainFrame = (MainFrame*)s_pMainFrame;
//	TwoDWidget *p2DWidget = (TwoDWidget*)s_p2DWidget;
//	GLWidget *pGLWidget = (GLWidget*)s_pGLWidget;

//	if(pMainFrame->m_centralWidget->currentIndex()==0) p2DWidget->contextMenuEvent(event);
