...
//		m_pctrlWingMethod1->setEnabled(false);
	...
	bool bWarning = false;
	for (i=0; i<m_pBlade->m_NPanel-1; i++)
	{
		if(m_pBlade->m_NXPanels[i]!= m_pBlade->m_NXPanels[i+1])
			{
...
void WPolarDlg::OnPlaneInertia()
{
	...
	if(m_pctrlPlaneInertia->isChecked())
	{
		...
		{
			m_pctrlWeight->setValue(m_pBlade->TotalMass() * pMainFrame->m_kgtoUnit);
			m_pctrlXCmRef->setValue(m_pBlade->m_CoG.x * pMainFrame->m_mtoUnit);
			m_pctrlZCmRef->setValue(m_pBlade->m_CoG.z * pMainFrame->m_mtoUnit);
			m_Weight    = m_pBlade->TotalMass();
			m_CoG.x     = m_pBlade->m_CoG.x;
			m_CoG.z     = m_pBlade->m_CoG.z;
		}
	}
	else
	{
...
void WPolarDlg::SetupLayout()
{
	...
	QGroupBox *FlightDataBox = new QGroupBox(tr("Plane and Flight Data"));
	{
		...
		m_pctrlQInf    = new NumberEdit();
		m_pctrlQInf->setValue(10.05);
		m_pctrlQInf->setMinimum(0.0);
		m_pctrlAlpha   = new NumberEdit(NumberEdit::Standard, 2);
		m_pctrlAlpha->setValue(1);
		m_pctrlBeta    = new NumberEdit(NumberEdit::Standard, 2);
		...
		QLabel *lab7 = new QLabel(QString::fromUtf8("deg"));
		QLabel *lab8 = new QLabel(QString::fromUtf8("deg"));
	...
	QGroupBox *InertiaBox = new QGroupBox(tr("Inertia properties"));
	{
		...
		m_pctrlWeight  = new NumberEdit(NumberEdit::Standard, 3);
		m_pctrlWeight->setValue(1.234);
		m_pctrlWeight->setMinimum(0.0);
		m_pctrlXCmRef  = new NumberEdit(NumberEdit::Standard, 3);
		m_pctrlXCmRef->setValue(100);
		m_pctrlZCmRef  = new NumberEdit(NumberEdit::Standard, 3);
		m_pctrlZCmRef->setValue(100);
...
//_________________________Main Layout
	...
	QGroupBox *AeroDataGroup = new QGroupBox(tr("Aerodynamic Data"));
	{
		...
		m_pctrlViscosity = new NumberEdit(NumberEdit::Scientific, 3);
		m_pctrlViscosity->setValue(1.500e-5);
		...
		m_pctrlDensity->setMinimum(0.0);
		m_pctrlViscosity->setMinimum(0.0);
	...
	QGroupBox *GroundGroup =  new QGroupBox(tr("Ground Effect"));
	{
		...
		m_pctrlHeight = new NumberEdit(NumberEdit::Standard, 2);
...
void WPolarDlg::SetReynolds()
{
	...
	if (m_WPolarType ==FIXEDSPEEDPOLAR)
	{
		double RRe = m_pBlade->m_TChord[0] * m_QInf/m_Viscosity;
		ReynoldsFormat(str, RRe);
		strange = tr("Root Re =");
		m_pctrlRRe->setText(strange+str);

		double SRe = m_pBlade->m_TChord[m_pBlade->m_NPanel] * m_QInf/m_Viscosity;
	...
	{
		double area;
		if (m_RefAreaType==1) area =m_pBlade->m_PlanformArea;
		else area = m_pBlade->m_ProjectedArea;
		...
		double RRe = m_pBlade->m_TChord[0] * QCl/m_Viscosity;
		ReynoldsFormat(str, RRe);
		strange = tr("Root Re.sqrt(Cl) =");
		m_pctrlRRe->setText(strange+str);

		double SRe = m_pBlade->m_TChord[m_pBlade->m_NPanel] * QCl/m_Viscosity;
		ReynoldsFormat(str, SRe);
