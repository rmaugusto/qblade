...
void ManageUFOsDlg::InitDialog(QString &UFOName)
{
	...
	QString str = tr("Rt-Tip Sweep") +QString::fromUtf8(" (deg)");
	...
	if(m_pUFOModel->rowCount())
	{
		...
		m_pBlade = pMiarex->GetWing(UFOName);
		if(m_pBlade) m_pPlane = NULL;
...
void ManageUFOsDlg::OnRename()
{
	...
	else if (m_pBlade) pMiarex->RenameUFO(m_pBlade->m_WingName);
...
void ManageUFOsDlg::OnDelete()
{
	...
	else         pMainFrame->DeleteWing(m_pBlade, false);
	...
	if(m_pUFOModel->rowCount()>0)
	{
		...
		m_pBlade = pMiarex->GetWing(UFOName);
		if(m_pBlade) m_pPlane = NULL; //not necessary...
		else        m_pPlane = pMiarex->GetPlane(UFOName);
	}
	else
	{
...
void ManageUFOsDlg::OnDescriptionChanged()
{
	...
	{
		m_pBlade->m_WingDescription = m_pctrlDescription->toPlainText();
	}
	MainFrame *pMainFrame = (MainFrame*)s_pMainFrame;
	pMainFrame->SetSaveState(false);
}
...
void ManageUFOsDlg::OnUFOClicked(QModelIndex index)
{
	...
	if(m_pBlade)
	{
		...
		m_pctrlDescription->setText(m_pBlade->m_WingDescription);
	}
	else
	{
