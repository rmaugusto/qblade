...
void PanelAnalysisDlg::ComputeFarField(double QInf, double Alpha0, double AlphaDelta, int nval)
{
	...
	for (q=0; q<nval;q++)
	{
		...
		strong = tr("        Calculating point ") + QString("%1").arg(alpha,7,'f',2)+QString::fromUtf8("deg....\n");
		AddString(strong);
...
void PanelAnalysisDlg::ComputePlane(double Alpha, double QInf, int qrhs)
{
	...
	if(QInf >0.0) 
	{
		...
		m_GCm *= 1.0 / m_pWPolar->m_WArea /m_pBlade->m_MAChord;
		m_VCm *= 1.0 / m_pWPolar->m_WArea /m_pBlade->m_MAChord;
		m_ICm *= 1.0 / m_pWPolar->m_WArea /m_pBlade->m_MAChord;
...
/*	if(m_pWPolar && (m_pWPolar->m_bTiltedGeom || m_pWPolar->m_bWakeRollUp))
	...
	m_b3DSymetric = m_pBlade->m_bSymetric;
...
void PanelAnalysisDlg::SetFileHeader()
{
	...
	else if(m_pBlade) out << m_pBlade->WingName();
	out << "\n";
...
/*					if(m_pWPolar->m_bGround)
					{
						AA  = pPanel->VA;
						BB  = pPanel->VB;
						AAG = m_pWakePanel[pw].VA;
						BBG = m_pWakePanel[pw].VB;
						AA.z  = -AA.z - 2.0*m_pWPolar->m_Height;
						BB.z  = -BB.z - 2.0*m_pWPolar->m_Height;
						AAG.z = -AAG.z - 2.0*m_pWPolar->m_Height;
						BBG.z = -BBG.z - 2.0*m_pWPolar->m_Height;
...
/*						if(m_pWPolar->m_bGround)
						{
							...
							AAG = m_pWakePanel[pw+1].VA;
							BBG = m_pWakePanel[pw+1].VB;
							AA.z  = -AA.z - 2.0*m_pWPolar->m_Height;
							BB.z  = -BB.z - 2.0*m_pWPolar->m_Height;
							AAG.z = -AAG.z - 2.0*m_pWPolar->m_Height;
							BBG.z = -BBG.z - 2.0*m_pWPolar->m_Height;
		...
		for(k=0; k<m_ppSurface[j]->m_NYPanels; k++)
		{
			...
			else
			{
				...
				for(l=0; l<m_ppSurface[j]->m_NXPanels; l++)
				{
					...
					{
						C = m_pPanel[p].CtrlPt;
						C.x = m_pBlade->m_PlanformSpan * 100.0;
			...
			if(m_pWPolar->m_bViscous)
			{
				...
				PCd    = GetVar(&g_polarStore, 2, m_ppSurface[j]->m_pFoilA, m_ppSurface[j]->m_pFoilB, Re, Cl, tau, bOutRe, bError);
...
bool PanelAnalysisDlg::ComputeTrimmedConditions()
{
	...
	strong += QString("Alpha=%1").arg(m_AlphaEq,0,'f',5) + QString::fromUtf8("deg") +"\n";
	AddString(strong);
...
//	mac = m_pBlade->m_MAChord;
	...
	if(m_pPlane)
	{
		    ...
		    ( m_pWPolar->m_bAVLControls && fabs(m_pWPolar->m_MaxControl[0])>PRECISION))
		{
			...
			for(p=0; p<m_pBlade->m_MatSize; p++)
			{
				(m_pBlade->m_pPanel+p)->RotateBC(m_pPlane->WingLE(0), Quat);
			}
		}

		pos = m_pBlade->m_MatSize;
		NCtrls = 1;
	}
