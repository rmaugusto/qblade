...
void LLTAnalysis::LLTInitCl(double &QInf, double const Alpha)
{
	...
	for (k=1; k<s_NLLTStations; k++)
	{
		...
		m_pBlade->GetFoils(&pFoil0, &pFoil1, yob*m_pBlade->m_PlanformSpan/2.0, tau);
		m_Re[k] = m_Chord[k] * QInf /m_pWPolar->m_Viscosity;
		m_Cl[k] = GetCl(&g_polarStore, pFoil0, pFoil1, m_Re[k], Alpha + m_Ai[k] + m_Twist[k], tau, bOutRe, bError);
	}
	...
	if(m_pWPolar->m_WPolarType==FIXEDLIFTPOLAR)
	{
		...
		for (k=1; k<s_NLLTStations; k++)
		{
			Lift += Eta(k) * m_Cl[k] * m_Chord[k] /m_pBlade->m_PlanformSpan;
		}
		if(Lift<=0.0) return;
...
double LLTAnalysis::Eta(int m)
{
	...
	return PI/2.0/(double)m_pBlade->m_NStation * sin((double)m*PI/(double)m_pBlade->m_NStation) ;
}
...
void LLTAnalysis::LLTComputeWing(double QInf, double Alpha, QString &ErrorMessage)
{
	...
	for (m=1; m<s_NLLTStations; m++)
	{
		...
		m_Cl[m]     = GetCl(&g_polarStore, pFoil0, pFoil1, m_Re[m], Alpha+m_Ai[m]+m_Twist[m], tau, bOutRe, bError);
		if(bOutRe) bPointOutRe = true;
		if(bError) bPointOutAlpha = true;

		m_PCd[m]    = GetCd(&g_polarStore, pFoil0, pFoil1, m_Re[m], Alpha+m_Ai[m]+m_Twist[m], tau, m_pBlade->m_AR, bOutRe, bError);
		if(bOutRe) bPointOutRe = true;
		if(bError) bPointOutAlpha = true;

		m_ICd[m]    = -m_Cl[m] * (m_Ai[m]* PI/180.0);

		m_XTrTop[m] = GetXTr(&g_polarStore, pFoil0, pFoil1, m_Re[m], Alpha+m_Ai[m] + m_Twist[m], tau, true, bOutRe, bError);
		if(bOutRe) bPointOutRe = true;
		if(bError) bPointOutAlpha = true;

		m_XTrBot[m] = GetXTr(&g_polarStore, pFoil0, pFoil1, m_Re[m], Alpha+m_Ai[m]+m_Twist[m], tau, false, bOutRe, bError);
		if(bOutRe) bPointOutRe = true;
		if(bError) bPointOutAlpha = true;

		m_CmAirf[m] = GetCm(&g_polarStore, pFoil0, pFoil1, m_Re[m], Alpha+m_Ai[m]+m_Twist[m], tau, bOutRe, bError);
		if(bOutRe) bPointOutRe = true;
		if(bError) bPointOutAlpha = true;

		m_XCPSpanRel[m] = GetXCp(&g_polarStore, pFoil0, pFoil1, m_Re[m], Alpha+m_Ai[m]+m_Twist[m], tau, bOutRe, bError);

		if(fabs(m_XCPSpanRel[m])<0.000001)
		{
			//plr mesh was generated prior to v3.15, i.e., without XCp calculations
			Cm0 = GetCm0(&g_polarStore, pFoil0, pFoil1, m_Re[m],tau, bOutRe, bError);
...
//		arad = (s_Alpha-m_Ai[m])*PI/180.0;
		c4   = m_pBlade->C4(yob, m_pWPolar->m_CoG.x)/m_Chord[m];
		zpos = m_pBlade->ZPos(yob*m_pBlade->m_PlanformSpan/2.0)/m_Chord[m];
		...
		if(bPointOutAlpha)
		{
			ErrorMessage = QString(QObject::tr("       Span pos = %1 ")).arg(cos(m*PI/s_NLLTStations)*m_pBlade->m_PlanformSpan/2.0*m_mtoUnit,9,'f',2);
	...
	m_CL            =  Integral0   * m_pBlade->m_AR /m_pBlade->m_PlanformSpan;
	m_InducedDrag   =  InducedDrag * m_pBlade->m_AR /m_pBlade->m_PlanformSpan  * PI / 180.0;
	m_ViscousDrag   =  ViscousDrag / m_pBlade->m_GChord;

	m_VYm = ViscousYawingMoment /m_pBlade->m_GChord;
	m_IYm = InducedYawingMoment /m_pBlade->m_PlanformSpan * PI * m_pBlade->m_AR /180.0;
	m_GYm = m_VYm + m_IYm;
//	m_GCm = PitchingMoment / m_GChord / m_MAChord;
	m_VCm = VCm / m_pBlade->m_GChord / m_pBlade->m_MAChord;
	m_ICm = ICm / m_pBlade->m_GChord / m_pBlade->m_MAChord;
	m_GCm = m_VCm + m_ICm;

	m_GRm = -Integral1   * m_pBlade->m_AR /m_pBlade->m_PlanformSpan;

	if(m_CL !=0.0)	m_XCP = Integral2 * m_pBlade->m_AR /m_pBlade->m_PlanformSpan/m_CL;
	else           m_XCP = 0.0;
	if(m_pBlade->m_bSymetric) m_YCP = 0.0;
	else                     m_YCP = m_pBlade->m_AR/m_CL * Integral1;
...
bool LLTAnalysis::LLTSetLinearSolution(double Alpha)
{
	...
	for (i=1; i<s_NLLTStations; i++)
	{
		...
		for (j=1; j<s_NLLTStations; j++)
		{
			...
			aij[p]  = snt0 + c*PI/m_pBlade->m_PlanformSpan/2.0* fj*snt0/st0;
		}
		yob   = cos(i*PI/s_NLLTStations);
		m_pBlade->GetFoils(&pFoil0, &pFoil1, yob*m_pBlade->m_PlanformSpan/2.0, tau);
		a0 = GetZeroLiftAngle(&g_polarStore, pFoil0, pFoil1, m_Re[i], tau);
	...
	for (i=1; i<s_NLLTStations; i++)
	{
		...
		yob   = cos(i*PI/s_NLLTStations);
		m_pBlade->GetFoils(&pFoil0, &pFoil1, yob*m_pBlade->m_PlanformSpan/2.0, tau);
		GetLinearizedPolar(&g_polarStore, pFoil0, pFoil1, m_Re[i], tau, a0, slope);
		a0 = GetZeroLiftAngle(&g_polarStore, pFoil0, pFoil1, m_Re[i], tau);//better approximation ?
...
double LLTAnalysis::Sigma(int m)
{
	...
	return PI/8.0/(double)m_pBlade->m_NStation * sin(2.*(double)m*PI/(double)m_pBlade->m_NStation) ;
}
...
double LLTAnalysis::Beta(int m, int k)
{
	...
	double fk = (double)k;
	double fm = (double)m;
	double fr = (double)m_pBlade->m_NStation;
...
double LLTAnalysis::AlphaInduced(int k)
{
	...
	for (int m=1; m<m_pBlade->m_NStation; m++)
	{
		ai += Beta(m,k) * m_Cl[m] * m_Chord[m]/m_pBlade->m_PlanformSpan;
	}
	return ai;
}
...
int LLTAnalysis::LLTIterate(double &QInf, double Alpha)
{
	...
	while(iter<m_IterLim && !m_bSkip)
	{
		...
		if(m_pWPolar->m_WPolarType==FIXEDLIFTPOLAR)
		{
			Lift *= m_pBlade->m_AR / m_pBlade->m_PlanformSpan;
...
void LLTAnalysis::LLTInitialize(double QInf)
{
	...
	if(m_pWPolar->m_WPolarType==FIXEDLIFTPOLAR)	m_QInf0 = sqrt(2.*m_pWPolar->m_Mass* 9.81 /m_pWPolar->m_Density/m_pBlade->m_PlanformArea);
	else                                    m_QInf0 = 0.0;

	m_bConverged = true;
	m_bWingOut = false;

	m_pBlade->ComputeChords(s_NLLTStations, m_Chord, m_Offset, m_Twist);
	for (k=0; k<=s_NLLTStations; k++)
	{
		y   = cos(k*PI/s_NLLTStations)* m_pBlade->m_PlanformSpan/2.0;
		m_Twist[k] = m_pBlade->Twist(y);
	}

	for (k=0; k<=s_NLLTStations; k++)
	{
		m_SpanPos[k] = m_pBlade->m_PlanformSpan/2.0 * cos((double)k*PI/s_NLLTStations);
		m_Re[k] = m_Chord[k] * QInf/m_pWPolar->m_Viscosity;
	}
