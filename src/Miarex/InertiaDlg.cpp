...
void InertiaDlg::ComputeBodyAxisInertia()
{
	...
	else if(m_pBlade) m_pBlade->ComputeBodyAxisInertia();
...
void InertiaDlg::ComputeInertia()
{
	...
	if(m_pBlade)
	{
		m_pBlade->m_VolumeMass = m_VolumeMass;
		if(m_pBlade->m_VolumeMass>PRECISION) m_pBlade->ComputeVolumeInertia(m_VolumeCoG, m_CoGIxx, m_CoGIyy, m_CoGIzz, m_CoGIxz);
	}
	else if(m_pBody)
	{
...
void InertiaDlg::InitDialog()
{
	...
	if(m_pBlade)
	{
		m_VolumeMass = m_pBlade->m_VolumeMass;
		m_NMass = m_pBlade->m_NMass;
		for(int i=0; i<m_pBlade->m_NMass; i++)
		{
			m_MassValue[i] = m_pBlade->m_MassValue[i];
			m_MassPosition[i].Copy(m_pBlade->m_MassPosition[i]);
			m_MassTag[i]   = m_pBlade->m_MassTag[i];
		}
		m_pctrlVolumeMass->setValue(m_pBlade->m_VolumeMass * pMainFrame->m_kgtoUnit); //we only display half a wing, AVL way
		m_pctrlVolumeMassLabel->setText(tr("Wing Mass:"));
		m_pctrlWingInertia->setEnabled(true);
		setWindowTitle(tr("Inertia properties for ")+m_pBlade->m_WingName);
	}
	else if (m_pBody)
	{
...
void InertiaDlg::OnExportToAVL()
{
	...
	//
	if (!m_pBlade && !m_pBody && !m_pPlane) return;
	QString filter =".mass";
	...
	else if(m_pBlade) FileName = m_pBlade->m_WingName;
	...
	else if(m_pBlade)  out << "#   "+m_pBlade->m_WingName+"\n";
...
void InertiaDlg::OnOK()
{
	...
	if(m_pBlade)
	{
		...
		for(i=0; i< MAXMASSES; i++)
		{
			if(m_MassValue[i]>PRECISION)
			{
				m_pBlade->m_MassValue[j] = m_MassValue[i];
				m_pBlade->m_MassPosition[j].Copy(m_MassPosition[i]);
				m_pBlade->m_MassTag[j]   = m_MassTag[i];
				j++;
			}			
		}
		m_pBlade->m_NMass = j;
...
void InertiaDlg::SetupLayout()
{
	...
	QGroupBox *ObjectMassBox = new QGroupBox(tr("Object Mass - Volume only, excluding point masses"));
	{
		...
		m_pctrlVolumeMass = new NumberEdit(NumberEdit::Standard, 3);
		m_pctrlVolumeMass->setValue(1);
		...
		QGroupBox *CoGBox = new QGroupBox(tr("Center of gravity"));
		{
			...
			m_pctrlXCoG = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlYCoG = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlZCoG = new NumberEdit(NumberEdit::Standard, 3);
		...
		QGroupBox *ResultsBox = new QGroupBox(tr("Inertia in CoG Frame"));
		{
			...
			m_pctrlCoGIxx = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlCoGIxx->setValue(1);
			m_pctrlCoGIyy = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlCoGIyy->setValue(1.2);
			m_pctrlCoGIzz = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlCoGIzz->setValue(-1.5);
			m_pctrlCoGIxz = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlCoGIxz->setValue(4.2);
...
//	m_pctrlMassView->setEditTriggers(QAbstractItemView::NoEditTriggers);
	...
	QGroupBox *TotalMassBox = new QGroupBox(tr("Total Mass = Volume + point masses"));
	{
		...
		m_pctrlTotalMass        = new NumberEdit(NumberEdit::Standard, 3);
		m_pctrlTotalMass->setValue(1);
		...
		QGroupBox *TotalCoGBox = new QGroupBox(tr("Center of gravity"));
		{
			...
			m_pctrlXTotalCoG = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlYTotalCoG = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlZTotalCoG = new NumberEdit(NumberEdit::Standard, 3);
		...
		QGroupBox *TotalInertiaBox = new QGroupBox(tr("Inertia in CoG Frame"));
		{
			...
			m_pctrlTotalIxx = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlTotalIxx->setValue(1);
			m_pctrlTotalIyy = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlTotalIyy->setValue(1.2);
			m_pctrlTotalIzz = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlTotalIzz->setValue(-1.5);
			m_pctrlTotalIxz = new NumberEdit(NumberEdit::Standard, 3);
			m_pctrlTotalIxz->setValue(4.2);
