...
void FoilGeomDlg::SetupLayout()
{
	...
	m_pctrlThickness =new NumberEdit;
	...
	m_pctrlXThickness = new NumberEdit;
	...
	m_pctrlCamber->setAutomaticPrecision(2);
	m_pctrlXCamber->setAutomaticPrecision(2);
	m_pctrlThickness->setAutomaticPrecision(2);
	m_pctrlXThickness->setAutomaticPrecision(2);
...
/*		for (int k=0; k<pXFoil->n;k++){
	...
	if(!m_bApplied)
	{
		double thickness = m_pctrlThickness->getValue()/100.0;
		double camber    = m_pctrlCamber->getValue()/100.0;
	...
	if(!m_bAppliedX)
	{
		double Xthickness = m_pctrlXThickness->getValue()/100.0;
		double Xcamber    = m_pctrlXCamber->getValue()/100.0;
