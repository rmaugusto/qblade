...
void FoilPolarDlg::SetupLayout()
{
	...
	QGroupBox *PlaneDataGroup = new QGroupBox(tr("Plane Data"));
	{
		...
		m_pctrlChord = new NumberEdit(NumberEdit::Standard, 3);
		m_pctrlMass = new NumberEdit(NumberEdit::Standard, 3);
		m_pctrlSpan = new NumberEdit(NumberEdit::Standard, 3);
	...
	m_pctrlReynolds = new NumberEdit();
...
//    ReMachLayout->addLayout(Type2DataLayout);
	...
	m_pctrlTopTrans = new NumberEdit();
	m_pctrlBotTrans = new NumberEdit();
...
//    mainLayout->addWidget(TypeGroup);
	...
	m_pctrlTopTrans->setAutomaticPrecision(2);
	m_pctrlTopTrans->setMinimum(0.0);
	m_pctrlTopTrans->setMaximum(1.0);

	m_pctrlBotTrans->setAutomaticPrecision(2);
	m_pctrlBotTrans->setMinimum(0.0);
	m_pctrlBotTrans->setMaximum(1.0);


	m_pctrlNCrit->setAutomaticPrecision(3);
	m_pctrlNCrit->setMinimum(0.0);
	m_pctrlNCrit->setMaximum(1000000.0);


	m_pctrlReynolds->setAutomaticPrecision(0);
	m_pctrlReynolds->setMinimum(-1.0e10);
	m_pctrlReynolds->setMaximum(1.e10);


	m_pctrlMach->setAutomaticPrecision(2);
	m_pctrlMach->setMinimum(0.0);
	m_pctrlMach->setMaximum(1000.0);
...
void FoilPolarDlg::InitDialog()
{
    ...
    if(pXDirect && g_pCurFoil) m_FoilName = g_pCurFoil->getName();
	else                       m_FoilName = "";
...
void FoilPolarDlg::ReadParams()
{
    ...
    m_Mass = m_pctrlMass->getValue();
    m_Chord = m_pctrlChord->getValue();
    m_Span = m_pctrlSpan->getValue();
