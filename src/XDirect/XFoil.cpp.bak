...
bool XFoil::bldif(int ityp)
{
	...
	double dxi, slog, scc, scc_usa/*, scc_us1*//*, scc_us2*/;
...
//      include 'xfoil.inc'
	...
	double /*xbl, */ybl, cxmax, cymax,  txmax, tymax;
	double ycc,ytt;
	double arot, sbl;
...
//---------------------------------------------------
	...
	n=0;// arcds : so that current airfoil is not initialized
	...
	memset(dqdm, 0, sizeof(dqdm));	memset(delt, 0, sizeof(delt));
...
bool XFoil::InitXFoilGeometry(CFoil *pFoil)
{
	...
	{
        QString str = QObject::tr("Unrecognized foil format")+" "+pFoil->getName();
		WriteString(str);
		return false;
	}
}
...
//	double gap;
	...
	if(nb<2)
	{
		...
		msgBox.setStandardButtons(QMessageBox::Ok);
		msgBox.setWindowTitle(QXDirect::tr("Warning"));
		msgBox.setText(QObject::tr("PanGen: buffer airfoil not available."));
		msgBox.exec();
		n = 0;
		return;
	}
...
//----------------------------------------------------------------
	...
	double ut_a1, tt_x1, dt_x1, ut_x1, tt_x2, dt_x2, ut_x2/*, tt_xf*//*, dt_xf*//*, ut_xf*/;
	double ax_d1, ax_u1, ax_x1, ax_d2, ax_u2, ax_x2/*, ax_xf*/, ax_ms, ax_re;
	double z_ax, z_a1, z_t1, z_d1, z_u1, z_x1, z_a2, z_t2, z_d2, z_u2, z_x2/*, z_xf*/, z_ms, z_re;
...
void XFoil::flap()
{
	...
	double yb1/*, yb2*/, sb1p, sb1q, sb2p, sb2q;
...
//			if(real(dcn)*real(dcn)+imag(dcn)*imag(dcn) < ceps*ceps) break;
		...
		double dscmax = 0.0;
		for(ic=1; ic<=nc;ic++)
			dscmax = qMax( dscmax , fabs(sc[ic]-scold[ic]) );
...
void XFoil::Filter(double cfilt)
{
