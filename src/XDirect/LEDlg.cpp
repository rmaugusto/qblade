...
{
	m_pctrlLE->setMinimum(  0.0);
	m_pctrlLE->setMaximum(100.0);

	m_pctrlBlend->setMinimum(  0.001);
	m_pctrlBlend->setMaximum(100.0);
...
/*		for (int k=0; k<pXFoil->n;k++)
	...
	m_LErfac = m_pctrlLE->getValue();
	m_Blend = m_pctrlBlend->getValue()/100.0;
